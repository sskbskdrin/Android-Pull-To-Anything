...
public class MaterialProgressDrawable extends Drawable implements Animatable {
    ...
    private void setUp(final double diameter) {
        final int shadowYOffset = dp2px(Y_OFFSET);
        final int shadowXOffset = dp2px(X_OFFSET);
        int mShadowRadius = dp2px(SHADOW_RADIUS);
    ...
    private int dp2px(float dp) {
        return (int) (mParent.getContext().getResources().getDisplayMetrics().density * dp + 0.5f);
    }
    ...
    private void setupAnimators() {
        ...
        final Animation finishRingAnimation = new Animation() {
            public void applyTransformation(float interpolatedTime, Transformation t) {
                ...
                final float startTrim = ring.getStartingStartTrim() + (ring.getStartingEndTrim() - ring
                    .getStartingStartTrim()) * interpolatedTime;
                ring.setStartTrim(startTrim);
                final float rotation = ring.getStartingRotation() + ((targetRotation - ring.getStartingRotation()) *
                    interpolatedTime);
        ...
        final Animation animation = new Animation() {
            ...
            public void applyTransformation(float interpolatedTime, Transformation t) {
                ...
                final float minProgressArc = (float) Math.toRadians(ring.getStrokeWidth() / (2 * Math.PI * ring
                    .getCenterRadius()));
                ...
                final float endTrim = startingEndTrim + (minArc * START_CURVE_INTERPOLATOR.getInterpolation
                    (interpolatedTime));
                ring.setEndTrim(endTrim);
                final float startTrim = startingTrim + (MAX_PROGRESS_ARC * END_CURVE_INTERPOLATOR.getInterpolation
                    (interpolatedTime));
                ring.setStartTrim(startTrim);
                final float rotation = startingRotation + (0.25f * interpolatedTime);
                ring.setRotation(rotation);
                float groupRotation = ((720.0f / NUM_POINTS) * interpolatedTime) + (720.0f * (mRotationCount /
                    NUM_POINTS));
    ...
    private class OvalShadow extends OvalShape {
        ...
        public OvalShadow(int shadowRadius, int circleDiameter) {
            ...
            mRadialGradient = new RadialGradient(mCircleDiameter / 2, mCircleDiameter / 2, mShadowRadius, new
                int[]{FILL_SHADOW_COLOR, Color.TRANSPARENT}, null, Shader.TileMode.CLAMP);
