...
public class StoreHouseHeader extends View implements PullUIHandler {
	...
	public void initWithPointList(ArrayList<float[]> pointList) {
		...
		for (int i = 0; i < pointList.size(); i++) {
			...
			PointF startPoint = new PointF(dp2px(line[0]) * mScale, dp2px(line[1]) * mScale);
			PointF endPoint = new PointF(dp2px(line[2]) * mScale, dp2px(line[3]) * mScale);
	...
	public void onUIPositionChange(int dx, int dy, int offsetX, int offsetY, int status) {
		...
		float currentPercent = Math.min(1f, off / getRefreshExtent());
