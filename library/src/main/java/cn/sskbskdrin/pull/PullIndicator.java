...
public class PullIndicator {
    ...
    public float getResistance() {
        ...
        if (offset == 0) offset = Math.abs(mCurrentPosition.y);
        if (offset > mMaxRange) offset = mMaxRange;
        float scale = (mMaxRange - offset) * 1f / mMaxRange;
        if (!isIncreaseResistance && scale != 0) scale = 1;
        return mResistance / scale;
    }

    public void setResistance(float resistance) {
        if (resistance == 0) throw new IllegalArgumentException("resistance Cannot be zero");
    ...
    private void setOffset(float x, float y) {
        ...
        if (offX * mCurrentPosition.x < 0) x = -mCurrentPosition.x;
        if (offY * mCurrentPosition.y < 0) y = -mCurrentPosition.y;
        mOffsetX = (int) x;
        mOffsetY = (int) y;
    }
